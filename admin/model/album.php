...
function edit_album ($conn,$album) {
    $stmt = $conn->prepare("UPDATE album SET album_name= :album_name,category_id= :category_id,id_album= :id_album,album_date= :album_date,album_image= :album_image,id_artist= :id_artist WHERE id = :id");
    $stmt->bindParam(':album_name',$album["album_name"],PDO::PARAM_STR);
    $stmt->bindParam(':category_id',$album["category_id"],PDO::PARAM_INT);
...
function check_album_exist ($conn,$data,$edit = false) {
    if (!$edit) {
        ...
        $stmt = $conn->prepare("SELECT album_name FROM album WHERE album_name = :album_name AND id != :id");
