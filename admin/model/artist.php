...
function check_artist_exist ($conn,$data,$edit = false) {
    if (!$edit) {
        ...
        $stmt = $conn->prepare("SELECT artist_name FROM artist WHERE artist_name = :artist_name AND id_artist != :id_artist");
        $stmt->bindParam(':id_artist',$data["id"],PDO::PARAM_STR);
    }
...
function get_artist ($conn,$id) {
    $stmt = $conn->prepare("SELECT * FROM artist WHERE id_artist = :id_artist");
    $stmt->bindParam(":id_artist",$id,PDO::PARAM_STR);
    $stmt->execute();
...
function delete_artist ($conn,$id) {
    $stmt = $conn->prepare("DELETE FROM artist WHERE id_artist = :id_artist");
    $stmt->bindParam(":id_artist",$id,PDO::PARAM_STR);
    $stmt->execute();
